...
async def process_new_donor_message(
    channel_id: int, # <-- Добавлен аргумент
    message_id: int, # <-- Добавлен аргумент
    text: str,       # <-- Добавлен аргумент
    media_paths: list[str], # <-- Добавлен аргумент
    source_link: str # <-- Добавлен аргумент
):
    """
    ...
    original_text = text # Используем переданный текст как оригинальный
    image_url = media_paths[0] if media_paths else None # Берем первый медиафайл, если есть
    ...
    async for session in get_session():
        ...
        is_duplicate, reason = await deduplicator.check_for_duplicates(session, original_text, city.id) # Передаем session
        ...
        if is_duplicate:
            new_post = Post(
                ...
                image_url=image_url,
                is_duplicate=True,
                status="rejected_duplicate", # Обновлен статус
                city_id=city.id,
                donor_channel_id=donor_channel.id,
                original_message_id=message_id # Сохраняем ID оригинального сообщения
            )
        ...
        if link_count > config.MAX_LINKS_IN_POST:
            new_post = Post(
                ...
                image_url=image_url,
                is_duplicate=False,
                status="rejected_too_many_links", # Новый статус
                city_id=city.id,
                donor_channel_id=donor_channel.id,
        ...
        if is_advertisement:
            new_post = Post(
                ...
                image_url=image_url,
                is_duplicate=False,
                is_advertisement=True, # Устанавливаем флаг
                status="pending", # Всегда pending для рекламы
                city_id=city.id,
                donor_channel_id=donor_channel.id,
                original_message_id=message_id
            )
        ...
        if not processed_text.strip():
            ...
            new_post = Post( # Сохраняем отклоненный пост
                original_text=original_text,
                processed_text=None,
                image_url=image_url,
                is_duplicate=False,
                status="rejected_empty_after_link_removal", # Новый статус
                city_id=city.id,
                donor_channel_id=donor_channel.id,
                original_message_id=message_id
            )
        ...
        if not processed_text.strip():
            ...
            new_post = Post( # Сохраняем отклоненный пост
                ...
                image_url=image_url,
                is_duplicate=False,
                status="rejected_empty_after_cta_removal", # Новый статус
                city_id=city.id,
                donor_channel_id=donor_channel.id,
                original_message_id=message_id
            )
        ...
        # 7. Добавление кастомной подписи
        # Формируем адрес канала: если есть username, используем его, иначе - t.me/c/ID
        # Предполагаем, что donor_channel.title содержит @username или просто название
        channel_address_part = donor_channel.title.replace('@', '') if donor_channel.title.startswith('@') else str(abs(donor_channel.telegram_id))
        custom_signature = f"\n\n\t\t**❤️** **[Подпишись на {config.CUSTOM_SIGNATURE}]({source_link})**" # Используем source_link как ссылку на оригинал
        final_text += custom_signature

        # 8. Сохранение поста в БД
        new_post = Post(
            original_text=original_text,
            processed_text=final_text,
            image_url=image_url,
            is_duplicate=False,
            is_advertisement=False, # Явно False, если не было помечено как реклама
            status="pending",
            city_id=city.id,
            donor_channel_id=donor_channel.id,
            original_message_id=message_id
        ...
        # 9. Публикация или отправка на модерацию
        if city.auto_mode:
